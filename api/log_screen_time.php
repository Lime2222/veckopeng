...
if ($childMember['screen_budget_minutes'] === null) {
    jsonOut(['error' => 'Skärmtid är inte aktiverat för det här barnet.'], 400);
}
...
db()->prepare('
    INSERT INTO screen_logs (child_id, log_date, minutes)
    VALUES (?, ?, ?)
    ON CONFLICT (child_id, log_date)
    DO UPDATE SET minutes = EXCLUDED.minutes
')->execute([$childId, $date, $minutes]);
