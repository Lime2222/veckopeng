...
$screenRaw    = trim($_POST['screen_budget_minutes'] ?? '');
$screenBudget = $screenRaw === '' ? null : max(0, (int)$screenRaw);
...
db()->prepare('UPDATE children SET name = ?, weekly_amount = ?, avatar_color = ?, swish_number = ?, screen_budget_minutes = ? WHERE id = ?')
    ->execute([$name, max(0, $weeklyAmount), $color, $swishNumber, $screenBudget, $childId]);
